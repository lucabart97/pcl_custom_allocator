...
namespace pcl
{
  class PCL_EXPORTS RFFaceDetectorTrainer
  {
    private:
      ...
      std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > head_center_votes_;
      std::vector<std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > > head_center_votes_clustered_;
      std::vector<std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > > head_center_original_votes_clustered_;
      std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > angle_votes_;
      std::vector<float> uncertainties_;
      std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > head_clusters_centers_;
      std::vector<Eigen::Vector3f, tk::tk_allocator<Eigen::Vector3f> > head_clusters_rotation_;
